...
TrafficLens should understand what happened, why we believe it, and where
...
def build_report(db: Session, capture: CaptureModel) -> str:
    ...
    parts = [
        ...
        f"<title>TrafficLens Report — {_esc(capture.filename)}</title>",
        ...
        f"<b>Generated:</b> {generated} by TrafficLens</div></div></header>",
    ]
    ...
    parts.append(
        f"<footer>Report generated by TrafficLens from packet evidence — every alert "
